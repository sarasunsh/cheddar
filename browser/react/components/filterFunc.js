export function filterFunc(dataArr, filters){
    ...
    for (var i = 0; i < resultKeys.length; i++) {
        if (resultKeys[i].split('+').length === filterCount){
            realResult[resultKeys[i].slice(8)] = result[resultKeys[i]].map(obj => obj.smilyScore);
        }
    }
...
export function dataSplit(data){
    ...
    const result2 = {};
    for (let i=0; i < keys.length; i++){
        const labels = keys[i].split(' + '); // e.g. ['gender:male', 'age:18-30']

        if (!result1[labels[0]]){
            result1[labels[0]] = [ labels[1], data[keys[i]].reduce((a,b)=> a+b, 0)/data[keys[i]].length ] // compute average
        } else {
            result1[labels[0]].push( labels[1], data[keys[i]].reduce((a,b)=> a+b, 0)/data[keys[i]].length )
        }
        if (!result2[labels[1]]){
            result2[labels[1]] = [ labels[0], data[keys[i]].reduce((a,b)=> a+b, 0)/data[keys[i]].length  ]
        } else {
            result2[labels[1]].push( labels[0], data[keys[i]].reduce((a,b)=> a+b, 0)/data[keys[i]].length  )
        }
    }
